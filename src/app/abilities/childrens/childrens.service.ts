...
export const create = async (category_id: string, item_id: string, createDto: DataType): Promise<DataType[] | CustomError> => {
...
export const findAll = async (category_id: string, item_id: string): Promise<DataType[] | CustomError> => {
...
export const findOne = async (category_id: string, item_id: string, children_id: string): Promise<DataType[] | CustomError> => {
...
export const update = async (category_id: string, item_id: string, children_id: string, updateDto: DataType): Promise<DataType[] | CustomError> => {
    const docRef = collRef.doc(category_id).collection(collItemName).doc(item_id).collection(collChildrenName).doc(children_id);
...
export const remove = async (category_id: string, item_id: string, children_id: string): Promise<DataType[] | CustomError> => {
    const docRef = collRef.doc(category_id).collection(collItemName).doc(item_id).collection(collChildrenName).doc(children_id);
    const res = await docRef.delete();
    if (res) return new CustomError(200, `El documento #${children_id} se eliminó exitosamente de: /abilities/categories/${category_id}/${collItemName}/${item_id}/${collChildrenName}.` );
